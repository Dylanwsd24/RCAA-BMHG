...
from datasum import load_data6,load_data3
import numpy as np
import warnings
from model_sum import models
...
def main(args):
    go,gi, features, NS, simlar,labels, num_classes, train_idx, val_idx, test_idx = load_data3()
    # go, gi, features, NS, simlar, labels, num_classes, train_idx, val_idx, test_idx = load_data6()
    ns_num = 2
    ...
    go = [graph.to(args['device']) for graph in go]
    gi = [graph.to(args['device']) for graph in gi]
    ...
    for epoch in range(1000):
        ...
        logits, h = model(features,go,gi, simlar)
    ...
    plt.legend()
    plt.savefig("DC" + str(args['dataset']) + "分类图" + str('1') + ".png", dpi=1000,bbox_inches='tight')
    plt.show()
